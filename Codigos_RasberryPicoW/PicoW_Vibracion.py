...
vibracion_sensor_digital = Pin(8, Pin.IN)  
...
def monitorear_mqtt(client, uid, sensor_id, ip_broker):
    ...
    while True:
        try:
            ...
            if  vibracion_sensor_digital.value() == 0:
                estado = 1
            ...
            print(f"Valor digital: {vibracion_sensor_digital.value()}")

                
            if estado == 1:  
                mensaje = json.dumps({"sensor_id": sensor_id, "estado": estado})
                print(f"Vibración detectada, enviando mensaje: {mensaje}")
