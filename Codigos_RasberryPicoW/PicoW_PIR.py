...
sensor_PIR = Pin(22, Pin.IN, Pin.PULL_UP)
...
def monitorear_mqtt(client, uid, sensor_id, ip_broker):
    ...
    while True:
        try:
            ...
            # Leer el estado del sensor de apertura
            estado = 1 if sensor_PIR.value() == 1 else 0

                
            if estado == 1:  
                mensaje = json.dumps({"sensor_id": sensor_id, "estado": estado})
                print(f"Movimiento detectado, enviando mensaje: {mensaje}")
